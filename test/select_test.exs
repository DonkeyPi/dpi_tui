defmodule Ash.SelectTest do
  ...
  test "basic select check" do
    ...
    select(items: [0, 1])
    |> render()
    |> check("0", 0, @tc_selected)
    |> check("1", 1, @tc_normal)
    |> focused(true)
    |> render()
    |> check("0", 0, @tc_selected)
    |> check("1", 1, @tc_focused)
    |> enabled(false)
    |> render()
    |> check("0", 0, @tc_selected)
    |> check("1", 1, @tc_disabled)
    ...
    |> render()
    |> check("3", 0, @tc_selected)
    |> check("4", 1, @tc_normal)
    # continue with page jumps
    |> handle(@ev_kp_pup, {:item, 1, 1, {:nop, {1, 1}}})
    |> render()
    |> check("1", 0, @tc_selected)
    |> check("2", 1, @tc_normal)
